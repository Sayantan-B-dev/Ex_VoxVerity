...
export default function LiveMonitoring() {
  const duration = useDuration(liveSession.durationSec);
    ...
    <div className="animate-fade-in space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="font-mono text-[13px] text-text-secondary">
              {liveSession.number} · {liveSession.context}
            </p>
          </div>
        </div>
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-[13px] text-text-secondary">
            Speaker profile mismatch at {liveSession.speakerSimilarity}% · synthetic signal
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <Card className="flex flex-col items-center justify-center gap-2 p-6 xl:col-span-1">
          ...
          <RiskMeter value={liveSession.risk} />
          <p className="text-center text-[12px] text-text-secondary">
            {bandTag(liveSession.riskLevel)} risk · composite of 4 detection signals
          </p>
        </Card>
          ...
          <Card className="p-6">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="flex items-center gap-1.5 font-mono text-[12px] text-teal">
                <CircleDot className="size-3.5" /> streaming · {liveSession.source} · 16kHz
              </span>
            </div>
            <Waveform />
          </Card>
      ...
      <Card className="grid grid-cols-2 gap-4 p-5 sm:grid-cols-4">
        {[
          { label: "Session", value: liveSession.sessionState },
          { label: "Chunk Latency", value: `${liveSession.chunkLatencyMs}ms` },
          { label: "Queue Depth", value: String(liveSession.queueDepth) },
          { label: "Chunk Sequence", value: String(liveSession.chunkSequence) },
        ].map((m) => (
          <div key={m.label}>
