...
import { calls, dashboardStats, riskDistribution } from "@/lib/demo-data";
...
export default function CallsView() {
  ...
  const flagged = calls.filter((c) => c.outcome !== "Verified");
    ...
    <div className="animate-fade-in space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-text-secondary">
            Suspicious calls we've flagged for review — verify the safe ones, dismiss the rest.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:col-span-8">
          <Stat
            icon={Gauge}
            value={String(dashboardStats.threatsDetected)}
            label="Flagged today"
          ...
          <Stat
            icon={Ban}
            value={`$${Math.round(dashboardStats.lossesPreventedUsd / 1000)}K`}
          ...
          <Stat
            icon={Check}
            value={`${dashboardStats.falsePositiveRate}%`}
      ...
      {/* main row: flagged calls + spoof types */}
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        {/* calls table */}
        <Card className="p-6 lg:col-span-8">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <h2 className="text-[18px] font-semibold">Flagged Calls</h2>
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 flex items-center gap-1.5 text-[12px] text-text-secondary">
                      <Globe className="size-3.5" /> {c.syntheticLabel.replace(/_/g, " ")} ·{" "}
                      {c.outcome}
                    </p>
                  </div>
        ...
        <Card className="p-6 lg:col-span-4">
          ...
          {/* risk distribution */}
          <div className="mt-6">
            <p className="mb-3 text-[13px] font-medium text-text-secondary">Risk distribution</p>
            {riskDistribution.map((r) => (
              <div key={r.band} className="flex items-center justify-between text-[12px] py-1">
